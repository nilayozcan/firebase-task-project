...
export default function FollowingPage({ params: paramsInput }: { params: { userId: string } }) {
  ...
  useEffect(() => {
    ...
    if (idFromParams) {
      ...
      // console.error("FollowingPage: userId is missing from resolved params.");
    }
  ...
  const pageTitle = viewedUser ? `${viewedUser.name} Kullanıcısının Takip Ettikleri` : "Takip Edilenler";

  return (
    <AppLayout pageTitle={pageTitle}>
      <UserListPageClient userId={currentUserId} listType="following" />
    </AppLayout>
  );
}
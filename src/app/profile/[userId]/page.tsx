...
export default function UserProfilePage({ params: paramsInput }: { params: { userId: string } }) {
  const { currentUser, isLoading: isAppLoading } = useAppContext();
  ...
  useEffect(() => {
    ...
    if (idFromParams) {
      ...
      // console.error("UserProfilePage: userId is missing from resolved params.");
    }
  ...
  return (
    <AppLayout pageTitle="Kullanıcı Profili">
      <UserProfilePageClient userId={currentUserId} />
    </AppLayout>
  );
}